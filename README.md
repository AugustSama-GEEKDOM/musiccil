...
终端里的黑胶唱片机。封面会缩成像素画放在唱片中心，跟着盘一起转；整个界面的配色从当前封面里取，
换歌的时候配色和封面一起渐变过去。音频交给 mpv 放，所以是真在放歌，不是动画。
...
| 中文曲目 | 歌词页 | 播放列表 |
|---|---|---|
| ![中文曲目](docs/images/player-cn.png) | ![歌词页](docs/images/lyrics.png) | ![播放列表](docs/images/playlist.png) |

还额外带了一个 Windows 图形界面，用来搜歌和攒歌单：

![图形界面](docs/images/gui.png)

界面全部在终端里画，没有网页、没有 Electron，也没有浏览器内核。

## 接口服务（密钥与配额）

曲库数据来自聚合接口。客户端里不存密钥，用的是你自己那把：
...
密钥在站点上免费领：注册账号，自动生成，面板里复制，然后 `musiccil --setup` 存到本地。
站点还提供这些：

- 邮箱验证码注册、登录、改密码。
- 每个账号最多 10 把密钥，可以重置、停用。
- 配额按次算：调用一次扣一次。新账号有初始配额，每天第一次调用会自动补一份，
  不够了还有签到和兑换码。
- 面板里能看到剩余配额、今日和近 14 天用量、按接口的分布，以及最近的调用明细。

取数失败会退还这一次的配额。校验密钥的 `/api/key-info` 不扣配额。
...
| 入口 | 什么时候用 |
|---|---|
| 命令行 `musiccil` | 想听某首歌、要一份推荐歌单，或者让脚本 / AI 帮你放 |
| 图形界面 `gui/` 里的 exe | 想慢慢搜、慢慢挑，手动攒一份歌单再开播 |

图形界面自己不做播放，只管找歌和组歌单，点「播放」时把歌单交给终端播放器。
这样两边共用同一套播放逻辑，配色、歌词、系统媒体键的表现自然一致。
...
这一步跳不过去，没有密钥搜不了歌。仓库里也不含密钥，公开仓库里写死密钥等于把配额送人。
...
没配的时候不会闷声失败，会直接把上面几条打出来。想知道还剩多少配额，
去调用站的「控制台 → 用量统计」。

## 用法

下面这些都能直接喂给它：歌名、`平台:ID`、本地文件、目录、m3u、音频直链、歌单、排行榜、JSON 歌单。
...
挑完再并发解析音频、封面和歌词，10 首大概 0.3 秒。`--seed` 指定的那首排第一，其余自动补。
...
换歌时配色和封面会在 0.9 秒内渐变过去，不是啪一下硬切。切歌和快进快退另有 0.22 秒的
音频淡入淡出，声音不会突然断掉。这几个时长都能改：
...
- 一次工具调用跑完全程。如果拆成「搜一次 → 读结果 → 写歌单 → 再解析 → 再播放」，
  实测要 5 分钟，而命令本身只要 1 秒，时间几乎全花在来回上。这就是 `--book` 和 `--mix`
  存在的原因。
- 播放器要开成独立窗口。它是交互式 TUI，在 agent 会话里直接跑会进管道，用户什么也看不到。
  播放器发现输出不是终端时会自己开窗，直接调就行。
...
播放器是交互式界面，得有个真终端才画得出来。它认得出自己「看不见用户」的情况——标准输出被
管道接走了，或者虽然连着终端但窗口是隐藏的（agent 的 PTY 就长这样）——这时它会自己新建一个
终端窗口跑起来，当前命令立刻返回，顺手打印窗口 pid 和按键提示。窗口标题会跟着当前曲目变。
...
窗口默认是竖屏 50×42（约 515×848 像素），唱片大概占屏高的三分之一，留白比较舒服。
Windows 上优先用 `wt --size` 开窗，这样窗口本身就是竖的；没有 Windows Terminal 就退回
新建控制台、由播放器自己调尺寸。
...
界面的输出行数和宽度始终等于终端尺寸，否则终端会滚动或者错位，中英日韩混排则按
East Asian Width 算列宽。另外播放和绘制是分开的：mpv 在后台通过命名管道 IPC 收指令，
界面只读状态、自己画，所以换歌、跳转、音量都不会把界面卡住。
...
- 接口是第三方聚合服务，配额按包量算，返回的音频和封面地址也带时效，不适合长期缓存。
